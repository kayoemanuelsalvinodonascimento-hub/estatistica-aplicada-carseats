...
```{r boxplots, fig.cap="Box-plot das vendas, por qualidade de prateleira.", out.width="70%"}
# Colega 1:
...
# resultado$out
# resultado$group
# levels(as.factor(dados$ShelveLoc))
...
variaveis_quantitativas <- dados[
  sapply(dados, is.numeric)
]
...
tabela_modelo1 <- as.data.frame(
  summary_modelo1$coefficients
)

knitr::kable(tabela_modelo1, format = "pipe", digits = 3,
             caption = "Tabela de coeficientes do modelo inicial.")


tabela_metricas <- data.frame(
  Metrica = c("$R^2$", "$R^2$ ajustado", "Erro padrão residual", "Número de observações"),
  Valor = c(summary_modelo1$r.squared, summary_modelo1$adj.r.squared, summary_modelo1$sigma, length(summary_modelo1$residuals))
)
...
knitr::kable(
  ...
  format = "pipe",
  digits = 3,
  caption = "Métricas de ajuste do modelo inicial."
)
